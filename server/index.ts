...
app.post('/api/claude-content', claudeLimiter, authGuard, async (req: Request, res: Response) => {
  ...
  try {
    const rows = await sql`
      ...
      FROM "briefcast-auth" WHERE user_id = ${userId}
    `;
...
app.post('/api/push-airtable', authGuard, async (req: Request, res: Response) => {
  ...
  try {
    const createBaseRes = await fetch('https://api.airtable.com/v0/meta/bases', {
      ...
      body: JSON.stringify({
        name: sanitizeString(title, 100) || 'Briefcast Content Calendar',
        tables: [{
...
app.post('/api/push-slack', authGuard, async (req: Request, res: Response) => {
  const { slackWebhookUrl, title, items } = req.body;
  if (!slackWebhookUrl || !items) return res.status(400).json({ error: 'slackWebhookUrl and items required' });
  if (!Array.isArray(items) || items.length > 500) return res.status(400).json({ error: 'Invalid items array' });
  if (!slackWebhookUrl.startsWith('https://hooks.slack.com/')) return res.status(400).json({ error: 'Invalid Slack webhook URL' });

  const calTitle = sanitizeString(title, 100);

  const lines = items.map((item: any) =>
    `• *${sanitizeString(item.title, 200)}* (${item.channel} · ${item.format} · ${item.publish_date})\n  _${sanitizeString(item.hook, 300)}_`
  ).join('\n');

  const payload = {
    blocks: [
      { type: 'header', text: { type: 'plain_text', text: `📅 ${calTitle}`, emoji: true } },
      { type: 'section', text: { type: 'mrkdwn', text: `*${items.length} content items generated by Briefcast:*` } },
...
app.post('/api/notion-auth', authGuard, async (req: Request, res: Response) => {
  const userId = (req as any).verifiedUserId as string;
  try {
    const rows = await sql`SELECT notion_auth FROM "briefcast-auth" WHERE user_id = ${userId}`;
...
app.post('/api/notion-current-month', authGuard, async (req: Request, res: Response) => {
  const userId = (req as any).verifiedUserId as string;
  try {
    const rows = await sql`SELECT current_usage FROM "briefcast-auth" WHERE user_id = ${userId}`;
...
app.post('/api/notion-max-month', authGuard, async (req: Request, res: Response) => {
  const userId = (req as any).verifiedUserId as string;
  try {
    const rows = await sql`SELECT max_monthly_usage FROM "briefcast-auth" WHERE user_id = ${userId}`;
...
app.post('/api/stripe-user-subscription', authGuard, async (req: Request, res: Response) => {
  const userId = (req as any).verifiedUserId as string;
  try {
    const rows = await sql`SELECT stripe_subscription_id FROM "briefcast-auth" WHERE user_id = ${userId}`;
...
app.post('/api/user-integrations', authGuard, async (req: Request, res: Response) => {
  const userId = (req as any).verifiedUserId as string;
  try {
    const rows = await sql`SELECT hubspot_token, monday_token, monday_board_id, trello_api_key, trello_token, buffer_access_token FROM "briefcast-auth" WHERE user_id = ${userId}`;
    res.status(200).json({
...
app.listen(port, () => {
  console.log(`Briefcast server running on port ${port}`);
});
